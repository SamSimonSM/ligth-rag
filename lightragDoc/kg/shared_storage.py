    ...
    async def __aenter__(self) -> "UnifiedLock[T]":
        try:
            if self._lock is None:
                raise RuntimeError(f"UnifiedLock: self._lock is None for lock '{self._name}'")
...
_graph_db_lock: Optional[asyncio.Lock] = None

def get_graph_db_lock(enable_logging: bool = False) -> UnifiedLock:
    """Return unified graph database lock for ensuring atomic operations"""
    global _graph_db_lock

    # Se não for multiprocessado, garanta que o lock existe
    if not _is_multiprocess and _graph_db_lock is None:
