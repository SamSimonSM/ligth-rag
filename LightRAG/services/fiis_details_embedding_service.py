        ...
        try:
            ...
            for i in range(3):  # Tentativas com backoff exponencial
                try:
                    ...
                    print(f"Entidades mescladas com sucesso para ticker: {ticker}")
                    return True
                except Exception as e:
                    print(f"Tentativa {i+1} falhou ao mesclar entidades. Erro: {e}")
                    await asyncio.sleep(2.0 * (2 ** i))  # Espera crescente: 2s, 4s, 8s
            return None  # Se falhar todas as tentativas
        except Exception as e:
            print(f"Erro inesperado ao tentar mesclar entidades: {e}")
            return None
